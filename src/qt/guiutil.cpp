...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Concordia Cash address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Concordia Cash: URI
...
{
    // Convert concordia:// to concordia:
    //
    //    Cannot handle this later, because concordia:// will cause Qt to see the part after // as host,
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CDIA (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CDIA (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "CDIA.lnk";
}
...
{
    return GetAutostartDir() / "concordia.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a concordia.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=CDIA (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=CDIA (regtest)\n";
        else
            optionFile << "Name=CDIA\n";
