...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "concordiad -foo=bar
...
#else
    const char* pszModule = "concordia";
#endif
    if (pex)
        return strprintf(
...
#else
    // Unix
    return pathRet / ".concordia-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/concordia/sapling-spend.params";
        sapling_output = "/usr/share/concordia/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/concordia/sapling-spend.params";
            sapling_output = "/usr/local/share/concordia/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
