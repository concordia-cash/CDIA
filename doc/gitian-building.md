...
*Setup instructions for a gitian build of Concordia Cash Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Concordia Cash
...
to the Concordia Cash GitHub release page.
...
- [Building Concordia Cash Core](#building-concordia-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/concordia-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/concordia-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/concordia-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/concordia-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/concordia-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/concordia-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/concordia-cash/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/concordia-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/concordia-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/concordia-osx-*-build.assert
```
