...
void CNode::copyStats(CNodeStats& stats, const std::vector<bool>& m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (CDIA users should be well used to small numbers with many decimal places by now :)
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    while (nInbound >= nMaxConnections - MAX_OUTBOUND_CONNECTIONS) {
...
void CConnman::ThreadMessageHandler()
{
    ...
    while (!flagInterruptMsgProc) {
        ...
        for (CNode* pnode : vNodesCopy) {
            ...
            // Send messages
            if (!fSkipSendMessagesForMasternodes) {
