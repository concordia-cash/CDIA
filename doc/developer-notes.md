...
- [Development guidelines](#development-guidelines)
    - [General Concordia Cash Core](#general-concordia-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Concordia Cash Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Concordia Cash Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/concordia-cash/CDIA/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_concordia
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_concordia --log_level=test_suite
$ valgrind -v --leak-check=full src/concordiad -printtoconsole
```
...
# A coverage report will now be accessible at `./test_concordia.coverage/index.html`.
```

**Sanitizers**
Concordia Cash can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Concordia Cash Core, where everyone uses
...
pay attention to for reviewers of Concordia Cash Core code.

General Concordia Cash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Concordia Cash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Concordia Cash Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/concordia-cash/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with Concordia Cash's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:concordia-cash/CDIA.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `concordia-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `concordia-cli` and the GUI debug console use this table to determine how to
