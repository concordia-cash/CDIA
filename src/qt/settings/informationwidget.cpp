...
#include "qt/settings/informationwidget.h"
#include "qt/settings/forms/ui_informationwidget.h"
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open concordia.conf with default application"));
    });
