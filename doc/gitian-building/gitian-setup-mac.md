...
The build script we'll be using is contained in the Concordia Cash github repository ([contrib/gitian-build.py](https://github.com/concordia-cash/CDIA/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Concordia Cash repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/concordia-cash/CDIA/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, concordia-detached-sigs, and concordia GitHub repos.
