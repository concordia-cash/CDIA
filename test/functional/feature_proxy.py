...
"""Test concordiad with different proxy configuration.

Test plan:
- Start concordiad's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on concordiad side:
...
- Create concordiads that connect to them
- Manipulate the concordiads using addnode (onetry) an observe effects
...
class ProxyTest(ConcordiaTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: concordiad's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
