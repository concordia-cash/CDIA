...
`test/functional/test_framework/test_framework.py:ConcordiaTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/concordia_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_concordiad = config["components"].getboolean("ENABLE_BITCOIND")
    ...
    if not (enable_wallet and enable_utils and enable_concordiad):
        print("No functional tests to run. Wallet, utils, and concordiad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, keep_cache="rewrite"):
    # Warn if concordiad is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "concordiad"]) is not None:
            print("%sWARNING!%s There is already a concordiad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/concordiad' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/concordia-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie concordiads, we can apply a
...
class RPCCoverage():
    ...
    commands per `concordia-cli help` (`rpc_interface.txt`).
