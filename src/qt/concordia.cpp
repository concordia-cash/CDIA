...
{
    return QCoreApplication::translate("concordia-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in concordia.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in concordia.qrc)
...
/** Class encapsulating Concordia Cash Core startup and shutdown.
...
/** Main Concordia Cash application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // Concordia Cash: URIs or payment requests:
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. Concordia Cash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(concordia_locale);
    Q_INIT_RESOURCE(concordia);
    ...
    /// 6. Determine availability of data directory and parse concordia.conf
    ...
    // concordia: links repeatedly have their payment requests routed to this process:
