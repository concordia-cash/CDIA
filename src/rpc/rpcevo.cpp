...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The Concordia Cash address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The Concordia Cash address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The Concordia Cash address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The Concordia Cash address to use for masternode reward payments.\n"
...
// Allows to specify Concordia Cash address or priv key (as strings). In case of Concordia Cash address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid Concordia Cash address %s", strAddress));
    }
    return *keyID;
}
